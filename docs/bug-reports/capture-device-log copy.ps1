...
if (Test-Path $OutFile) {
    ...
    Get-Content $OutFile | ForEach-Object {
        $line = $_
        if ($line -match '(\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}\.\d{3}).*p_sua->call_state=(\d)') {
            "    $($matches[1])  call_state=$($matches[2])"
        }
        elseif ($line -match '(\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}\.\d{3}).*Gb28181Local: onStateUpdate.*liveState:(\d)') {
            $cur = $matches[2]
            if ($cur -ne $prev) { "    $($matches[1])  liveState=$cur"; $script:prev = $cur }
        }
    }
} else {
